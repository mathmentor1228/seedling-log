...
export function TextbookPaymentTab() {
  ...
  const unpaidByStudent = useMemo(() => {
    ...
    const grouped = new Map<string, { studentName: string; dists: Distribution[] }>();
    for (const d of unpaid) {
      if (!grouped.has(d.student_id)) {
        grouped.set(d.student_id, { studentName: d.student_name, dists: [] });
      }
        ...
        <div>
          ...
          <div className="space-y-3">
            {unpaidByStudent.map(group => {
              ...
              const anyBilled = group.dists.some(d => !!d.billed_at);
