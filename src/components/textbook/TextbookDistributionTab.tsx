...
import { Plus, Loader2, BookMarked, Send, ChevronsUpDown, Check, Users, CheckCircle2, Package, Clock, Copy, ChevronDown, MessageCircle, BookOpen, ShoppingBag, ShoppingCart } from 'lucide-react';
...
export function TextbookDistributionTab() {
  ...
  const fetchData = useCallback(async () => {
    const [distRes, orderRes, studentRes] = await Promise.all([
      ...
      // Fetch ALL orders (not just 입고완료) so teachers can assign at order time
      supabase.from('textbook_orders').select('*').order('created_at', { ascending: false }),
      supabase.from('students').select('id, name, parent_name, parent_phone').in('enrollment_status', ['재원', '재원예정']).order('name'),
    ]);
  ...
  // Realtime for payment status changes → teacher notification
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'textbook_distributions' }, (payload) => {
        ...
        // Notify when payment status changes to 수납완료
  ...
  // Assign students to a textbook (배정 등록)
  const handleAssign = async () => {
    ...
    if (error) { toast.error('배정 등록 실패'); console.error(error); }
      ...
      fetchData();
      setTimeout(() => {
        toast.success(`${names} 학생 배정 완료 (${selectedStudents.length}명)`);
      }, 300);
    }
    setCreating(false);
  };
  ...
  const generateSelfPurchaseMessage = (studentName: string, studentDists: Distribution[]) => {
    const bookLines = studentDists.map((d, i) => {
      const bookName = d.textbook_orders?.textbook_name || '교재';
      const subject = d.textbook_orders?.subject || '';
  ...
  const markBilled = async (distIds: string[]) => {
    ...
    for (const id of distIds) {
      await supabase.from('textbook_distributions').update({ billed_at: now, message_sent_at: now } as any).eq('id', id);
    }
    fetchData();
  };
  ...
  const handleSendMessage = async (studentId: string, type: 'payment' | 'selfpurchase') => {
    ...
    const msg = type === 'payment' ? generatePaymentMessage(studentName, studentDists) : generateSelfPurchaseMessage(studentName, studentDists);
  ...
  const handleConfirmResend = async () => {
    ...
    const studentDists = distributions.filter(d => d.student_id === studentId && d.payment_status === '미납');
    const studentName = studentDists[0]?.student_name || '';
    const msg = resendType === 'payment' ? generatePaymentMessage(studentName, studentDists) : generateSelfPurchaseMessage(studentName, studentDists);
    await navigator.clipboard.writeText(msg);
    const now = new Date().toISOString();
    await Promise.all(studentDists.map(d =>
      supabase.from('textbook_distributions').update({ message_resent_at: now } as any).eq('id', d.id)
    ));
  ...
  // Physical distribution confirmation
  const handleConfirmDistribution = async (dist: Distribution) => {
    ...
    if (error) { toast.error('배부 확인 실패'); console.error(error); }
      ...
      toast.success(`${dist.student_name} 학생에게 교재 배부 완료`);
      fetchData();
    }
  };

  // ===== Categorize distributions by workflow stage =====
  // Stage 1: 배정 완료 - 안내문자 미발송 (미납 + no billed_at)
  const stage1_assigned = useMemo(() => {
    return distributions.filter(d => d.payment_status === '미납' && !d.billed_at);
  }, [distributions]);

  // Stage 2: 청구 완료 - 수납 대기 (미납 + billed_at set)
  const stage2_billed = useMemo(() => {
    return distributions.filter(d => d.payment_status === '미납' && !!d.billed_at);
  }, [distributions]);

  // Stage 3: 수납 완료 - 배부 대기 (수납완료 + no distributed_confirmed_at)
  const stage3_paid = useMemo(() => {
    return distributions.filter(d => d.payment_status === '수납완료' && !d.distributed_confirmed_at);
  }, [distributions]);

  // Stage 4: 배부 완료
  const stage4_distributed = useMemo(() => {
    return distributions.filter(d => (d.payment_status === '수납완료' || d.payment_status === '개별구매') && !!d.distributed_confirmed_at);
  }, [distributions]);

  // 개별구매 - 배부 대기
  const selfPurchasePending = useMemo(() => {
    return distributions.filter(d => d.payment_status === '개별구매' && !d.distributed_confirmed_at);
  }, [distributions]);

  // Group stage1 by student
  const stage1ByStudent = useMemo(() => {
    const grouped = new Map<string, { studentName: string; parentName: string | null; dists: Distribution[] }>();
    for (const d of stage1_assigned) {
  ...
  // Group stage2 by student
  const stage2ByStudent = useMemo(() => {
    const grouped = new Map<string, { studentName: string; parentName: string | null; dists: Distribution[] }>();
    for (const d of stage2_billed) {
      if (!grouped.has(d.student_id)) grouped.set(d.student_id, { studentName: d.student_name, parentName: d.parent_name || null, dists: [] });
      grouped.get(d.student_id)!.dists.push(d);
    }
    return Array.from(grouped.values());
  ...
  // Available orders for assignment (all orders, show status badge)
  const availableOrders = useMemo(() => {
    return orders.filter(o => (o.quantity - (o.distributed_qty || 0)) > 0);
  }, [orders]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">교재 배정 → 학부모 안내 → 수납 → 배부 순서로 관리합니다.</p>
        <Dialog open={showDialog} onOpenChange={(open) => { setShowDialog(open); if (!open) { setSelectedStudents([]); setSelectedOrder(''); setDistQty('1'); setStudentSearch(''); } }}>
          ...
          <DialogContent>
            ...
            <DialogDescription className="text-xs">교재를 받을 학생을 배정합니다. 배정 후 학부모 안내문자를 발송하세요.</DialogDescription>
            <div className="space-y-4 mt-2">
                  ...
                  if (o && o.status !== '입고완료') {
                    return (
                      <p className="text-xs text-yellow-600 mt-1 flex items-center gap-1">
                        ...
                        이 교재는 아직 입고 전입니다 (현재: {o.status}). 배정은 가능하지만 실제 배부는 입고 후 가능합니다.
                      </p>
                    );
                  }
                  return null;
              ...
              {selectedOrder && selectedStudents.length > 0 && (
                <div className="space-y-1 p-3 rounded-md bg-muted/50">
                  <p className="text-sm text-foreground font-medium">
                    배정 요약: {selectedStudents.length}명 × {distQty}권 = 총 {selectedStudents.length * (parseInt(distQty) || 1)}권
                  </p>
              ...
              <Button onClick={handleAssign} disabled={creating} className="w-full">
                ...
                {selectedStudents.length > 1 ? `${selectedStudents.length}명 배정 등록` : '배정 등록'}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
      ...
      {/* ===== STAGE 1: 배정 완료 - 안내문자 미발송 ===== */}
      {stage1ByStudent.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
            <BookMarked className="w-4 h-4 text-blue-500" />
            ① 배정 완료 · 안내문자 대기 ({stage1_assigned.length}건 · {stage1ByStudent.length}명)
          </h3>
          <p className="text-xs text-muted-foreground mb-2">학생에게 교재가 배정되었습니다. 학부모에게 안내문자를 발송해주세요.</p>
          <div className="space-y-2">
            {stage1ByStudent.map(group => {
              const totalAmount = group.dists.reduce((sum, d) => sum + d.total_amount, 0);
              return (
                <Card key={group.studentName + '-s1'} className="p-4 border-l-4 border-l-blue-400">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-medium text-foreground">{group.studentName}</p>
                        {group.parentName && <span className="text-xs text-muted-foreground">(학부모: {group.parentName})</span>}
                        <Badge className="text-[10px] bg-blue-100 text-blue-700 border-blue-300 dark:bg-blue-900/30 dark:text-blue-300">배정완료</Badge>
                        {group.dists.length > 1 && <Badge variant="outline" className="text-[10px]">{group.dists.length}건</Badge>}
                      </div>
                      {group.dists.map(dist => (
                        <p key={dist.id} className="text-sm text-muted-foreground mt-1">
                          · {dist.textbook_orders?.textbook_name} · {dist.quantity}권 · {dist.total_amount.toLocaleString()}원
                        </p>
                      ))}
                      {group.dists.length > 1 && (
                        <p className="text-sm font-medium text-foreground mt-1">합계: {totalAmount.toLocaleString()}원</p>
                      )}
                      <p className="text-xs text-muted-foreground mt-1">배정일: {format(new Date(group.dists[0].created_at), 'yyyy-MM-dd')} · 담당: {group.dists[0].distributed_by_name}</p>
                    </div>
                    <div className="flex flex-col gap-1.5 shrink-0">
                      <Button size="sm" className="gap-1" onClick={() => handleSendMessage(group.dists[0].student_id, 'payment')}>
                        <Send className="w-3.5 h-3.5" />안내문자
                      </Button>
                      <Button size="sm" variant="outline" className="gap-1 text-muted-foreground" onClick={() => handleSendMessage(group.dists[0].student_id, 'selfpurchase')}>
      ...
      {stage2ByStudent.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
            ...
            ② 청구 완료 · 수납 대기 ({stage2_billed.length}건 · {stage2ByStudent.length}명)
          </h3>
          <p className="text-xs text-muted-foreground mb-2">학부모에게 안내문자가 발송되었습니다. 수납을 확인해주세요.</p>
          <div className="space-y-2">
            {stage2ByStudent.map(group => {
              const totalAmount = group.dists.reduce((sum, d) => sum + d.total_amount, 0);
              const latestSent = group.dists.reduce((latest, d) => {
                const t = d.message_sent_at;
                return t && (!latest || t > latest) ? t : latest;
              }, null as string | null);

              return (
                <Card key={group.studentName + '-s2'} className="p-4 border-l-4 border-l-yellow-400 bg-yellow-50/40 dark:bg-yellow-950/10">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-medium text-foreground">{group.studentName}</p>
                        <Badge className="text-[10px] bg-yellow-100 text-yellow-700 border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-300">
                          <Clock className="w-2.5 h-2.5 mr-0.5" />수납대기
                        </Badge>
                      ...
                      {group.dists.map(dist => (
                        <div key={dist.id} className="flex items-center justify-between mt-1.5">
                          ...
                          <div className="flex items-center gap-1">
                            <Button size="sm" variant="ghost" className="h-7 text-xs gap-1 text-orange-600 hover:text-orange-700" onClick={() => handleSelfPurchase(dist)}>
                              <ShoppingBag className="w-3 h-3" />개별구매
                            </Button>
                            <Button size="sm" variant="ghost" className="h-7 text-xs gap-1 text-green-600 hover:text-green-700" onClick={() => openPaymentConfirm(dist)}>
                      ...
                      {latestSent && (
                        <p className="text-xs text-yellow-700 dark:text-yellow-400 mt-1">
                          📨 발송: {format(new Date(latestSent), 'MM/dd HH:mm')}
                        </p>
                      )}
                    </div>
                    <div className="flex flex-col gap-1.5 shrink-0">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button size="sm" variant="outline" className="gap-1 text-xs px-2">
                            <Copy className="w-3 h-3" />재전송<ChevronDown className="w-3 h-3 ml-0.5" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-40">
                          <DropdownMenuItem onClick={() => handleSendMessage(group.dists[0].student_id, 'payment')} className="text-xs gap-2">
                            <Send className="w-3.5 h-3.5" />수납 안내
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleCopyAndOpenKakao(group.dists[0].student_id, 'payment')} className="text-xs gap-2">
                            <MessageCircle className="w-3.5 h-3.5" />카톡 열기
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
      ...
      {(stage3_paid.length > 0 || selfPurchasePending.length > 0) && (
        <div>
          <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
            ...
            ③ 수납 완료 · 배부 대기 ({stage3_paid.length + selfPurchasePending.length}건)
          </h3>
          <p className="text-xs text-muted-foreground mb-2">수납이 완료되었습니다. 학생에게 교재를 전달한 후 배부 확인 버튼을 눌러주세요.</p>
          <div className="space-y-2">
                  ...
                  <div className="flex-1">
                    ...
                    {dist.paid_at && (
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {dist.payment_status === '수납완료' ? '수납' : '처리'}: {format(new Date(dist.paid_at), 'MM/dd')}
                        {dist.confirmed_by && ` · ${dist.confirmed_by}`}
                        {dist.depositor_name && ` · 입금자: ${dist.depositor_name}`}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Button size="sm" className="gap-1.5" onClick={() => handleConfirmDistribution(dist)}>
                      <CheckCircle2 className="w-3.5 h-3.5" />교재배부
                    </Button>
                    <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-muted-foreground" onClick={() => handleRevertPayment(dist)} title="미납으로 변경">
      ...
      {stage4_distributed.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-1.5">
            ...
            ④ 배부 완료 ({stage4_distributed.length}건)
          </h3>
          <div className="space-y-2">
            {stage4_distributed.map(dist => (
              <Card key={dist.id} className="p-3 opacity-70">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="font-medium text-sm text-foreground">{dist.student_name}</p>
                  <Badge variant="outline" className="text-[10px] border-green-400 text-green-600">배부완료</Badge>
                  <span className="text-xs text-muted-foreground">
                    {dist.textbook_orders?.textbook_name} · {dist.total_amount.toLocaleString()}원
                  </span>
                  <span className="text-xs text-muted-foreground ml-auto">
                    배부: {dist.distributed_confirmed_by} · {dist.distributed_confirmed_at ? format(new Date(dist.distributed_confirmed_at), 'MM/dd') : ''}
                  </span>
                </div>
              </Card>
            ))}
      ...
      {distributions.length === 0 && (
        <p className="text-center text-sm text-muted-foreground py-12">배정 기록이 없습니다</p>
      )}
        ...
        <DialogContent>
          ...
          <div className="space-y-3 mt-2">
            ...
            <div>
              ...
              <Input value={depositorName} onChange={(e) => setDepositorName(e.target.value)} placeholder="입금자 이름" className="mt-1" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground">학부모 성함 (선택)</label>
              <Input value={parentNameInput} onChange={(e) => setParentNameInput(e.target.value)} placeholder="학부모 이름" className="mt-1" />
            </div>
          </div>
          <DialogFooter>
