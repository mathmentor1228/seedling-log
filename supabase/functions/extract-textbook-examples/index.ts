...
[공통 전처리 — 반드시 최우선 적용]
1) 모든 수학적 표현은 반드시 LaTeX 표기법으로 변환하세요:
   - 제곱근: $\\sqrt{4}$, $\\sqrt{81}$, $-\\sqrt{7}$
   - 분수: $\\frac{a}{b}$, $\\frac{\\sqrt{8}}{\\sqrt{16}}$
   - 거듭제곱: $a^{2}$, $cm^{3}$
   - 곱셈/나눗셈: $\\times$, $\\div$
   - 부등호: $<$, $>$, $\\leq$, $\\geq$
   - 파이: $\\pi$
   - 단위도 수식과 결합: $3\\sqrt{6}\\text{cm}$, $540\\text{cm}^3$ (단위에 \\text{} 사용)
2) OCR 오류 주의: "oot" → $\\sqrt{}$, "루트" → $\\sqrt{}$ 등 잘못 인식된 수식을 올바른 LaTeX로 교정하세요.
3) 머리글/바닥글/페이지 번호/워터마크 등 장식 텍스트는 무시하세요.
...
[객관식 문항 규칙 — 매우 중요!]
- 보기가 ①②③④⑤ 형태인 문제는 반드시 보기를 모두 포함하세요.
- 각 보기 안의 수식도 반드시 LaTeX($...$)로 정확히 감싸세요.
- 예시: "① $\\sqrt{40} = 4\\sqrt{10}$ ② $-2\\sqrt{7} = \\sqrt{(-2)^2 \\times 7} = \\sqrt{28}$"
- 보기에 "ㄱ, ㄴ, ㄷ, ㄹ, ㅁ, ㅂ" 기호가 있으면 모두 "ㄱ. ...", "ㄴ. ...", "ㄷ. ...", "ㄹ. ...", "ㅁ. ...", "ㅂ. ..." 형태로 추출하세요.
- ⚠️ "ㅁ"과 "ㅂ"도 절대 누락하지 마세요.

[표(Table) 추출 규칙 — 매우 중요!]
- 표가 포함된 문제는 question_text 안에 표를 마크다운 형식으로 정확히 재현하세요.
- 예시:
  "| 수 | 0 | 1 | 2 | 3 | 4 | 5 |\\n|---|---|---|---|---|---|---|\\n| 5.3 | 2.302 | 2.304 | 2.307 | 2.309 | 2.311 | 2.313 |\\n| 5.4 | 2.324 | 2.326 | 2.328 | 2.330 | 2.332 | 2.335 |"
- 제곱근표, 삼각비표, 통계표 등은 행과 열을 정확히 옮기세요.
- 표의 값에 수식이 있으면 LaTeX로 감싸세요.

[그림/도형 설명 규칙 — 매우 중요!]
- 그림, 도형, 그래프 등 **시각 요소가 포함된 문제**는 반드시 question_text 안에 해당 시각 요소를 **텍스트로 상세하게 묘사**하세요.
   - 도형: 꼭짓점 이름, 변 길이, 각도, 도형 종류를 명시
     예: "[그림] 직육면체: 밑면은 가로 $3\\sqrt{6}\\text{cm}$, 세로 $3\\sqrt{5}\\text{cm}$인 직사각형, 부피 $540\\text{cm}^3$"
   - 그래프: 함수식, 교점 좌표, 범위 등을 수식으로 명시
   - 삼각형/사각형 등: 꼭짓점 라벨, 변의 길이(LaTeX 수식), 각도 정보를 빠짐없이 기록
- has_illustration을 true로 설정하고, illustration_description에 간략 설명을 기록하세요.
- graph_data에 함수식 정보를 JSON으로 기록 (해당시)

[추출 규칙]
1) 문제 번호, 문제 텍스트, 정답, 해설을 빠짐없이 추출
2) 번호가 없는 '함께 풀기' 등은 problem_number에 해당 키워드를 기입 (예: "함께 풀기 1")
3) 수식은 반드시 LaTeX 표기법 사용 (예: $x^2 + 2x + 1$)
