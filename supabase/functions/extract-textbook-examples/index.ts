...
[그림/도형 설명 규칙 - 매우 중요!]
9) 그림, 도형, 그래프, 표 등 **시각 요소가 포함된 문제**는 반드시 question_text 안에 해당 시각 요소를 **텍스트로 상세하게 묘사**하세요.
   - 도형: 꼭짓점 이름, 변 길이, 각도 등을 명시 (예: "삼각형 ABC에서 AB=5, BC=3, ∠B=60°")
   - 그래프: 함수식, 교점 좌표, 범위 등을 수식으로 명시 (예: "함수 $y=x^2-4x+3$의 그래프가 x축과 만나는 점")
   - 표: 표 내용을 텍스트로 재현 (예: "| x | 1 | 2 | 3 |\\n| y | 2 | 5 | 10 |")
   - 그림 설명을 "[그림] ..." 형식으로 본문에 포함
10) illustration_description 필드에 그림의 간략 설명을 반드시 기록`;
}
