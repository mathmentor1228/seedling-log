    ...
    const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `당신은 수학 시험 채점 전문가입니다. 학생이 손으로 적은 노트 사진을 분석하여 각 문제의 답안을 판독하고 채점해야 합니다.

규칙:
1. 사진 속에서 문제 번호(Q1, Q2, 1번, 2번 등)에 해당하는 학생의 답을 찾아 판독하세요.
2. 각 문제의 정답과 비교하여 맞았는지 틀렸는지 판정하세요.
3. 손글씨가 불분명한 경우 최대한 유추하되, 판독 불가시 "unreadable"로 표시하세요.
