...
-- The unique index idx_assistant_tasks_unique_template uses expression columns
-- with COALESCE, so we cannot use ON CONFLICT ON CONSTRAINT.
-- Instead, we use a SELECT + INSERT with locking to prevent duplicates.

-- Drop any existing overloads
DROP FUNCTION IF EXISTS public.create_assistant_task(text, text, date, text, uuid);
DROP FUNCTION IF EXISTS public.create_assistant_task(text, text, timestamptz, text, uuid);

-- Create RPC: create-or-return assistant task
-- Uses explicit duplicate check matching the unique index expression.
...
-- Permissions
REVOKE ALL ON FUNCTION public.create_assistant_task(text, text, date, text, uuid) FROM PUBLIC;
