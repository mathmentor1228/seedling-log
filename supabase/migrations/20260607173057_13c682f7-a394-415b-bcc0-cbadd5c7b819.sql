...
CREATE OR REPLACE FUNCTION public.get_admin_kpis(_start_date date, _end_date date, _teacher_id uuid DEFAULT NULL::uuid, _subject text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'draft', draft_count,
    'submission_rate', CASE WHEN total_count > 0 THEN ROUND((submitted_count::numeric / total_count) * 100, 1) ELSE 0 END
  ) INTO _lesson_stats FROM lesson_counts;
  ...
  SELECT jsonb_build_object(
    'completed', hc.completed_count, 'partial', hc.partial_count, 'not_done', hc.not_done_count,
    'none_assigned', hc.none_assigned_count, 'total', hc.total_count,
    'pending_verification', pw.pending_count
  ) INTO _homework_stats FROM homework_counts hc, pending_hw pw;

  -- FIX: exclude 휴강 from attendance exception counts
  ...
  SELECT jsonb_build_object(
    '지각', late_count, '조퇴', early_leave_count, '인정결석', excused_absence_count,
    '무단결석', unexcused_absence_count, '보충불가', no_makeup_count,
    'total', late_count + early_leave_count + excused_absence_count + unexcused_absence_count + no_makeup_count
  ) INTO _attendance_stats FROM exception_counts;

  WITH test_counts AS (
    SELECT lr.subject::text AS subj,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('subject', subj, 'test_count', test_count, 'pass_count', pass_count, 'fail_count', fail_count)), '[]'::jsonb)
  INTO _test_stats FROM test_counts;

  -- FIX: exclude 휴강 from top exception students
  WITH student_exceptions AS (
    SELECT lr.student_id, s.name AS student_name,
      COUNT(*) FILTER (WHERE 
    ...
    ORDER BY exception_count DESC LIMIT 5
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object('student_id', student_id, 'student_name', student_name, 'exception_count', exception_count)), '[]'::jsonb)
  INTO _top_exception_students FROM student_exceptions;

  WITH teacher_stats AS (
    SELECT lr.teacher_id, p.full_name AS teacher_name,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'teacher_id', teacher_id, 'teacher_name', COALESCE(teacher_name, '미배정'),
    'total_lessons', total_lessons, 'submitted', submitted_count, 'draft', draft_count,
    'attendance_issues', attendance_issues, 'homework_issues', homework_issues, 'tests_count', tests_count
  )), '[]'::jsonb) INTO _teacher_breakdown FROM teacher_stats;
