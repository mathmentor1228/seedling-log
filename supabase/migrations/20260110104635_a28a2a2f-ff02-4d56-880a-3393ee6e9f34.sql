-- ========================================
-- 1. Enable pg_cron and pg_net extensions for scheduling
-- ========================================
CREATE EXTENSION IF NOT EXISTS pg_cron WITH SCHEMA extensions;
CREATE EXTENSION IF NOT EXISTS pg_net WITH SCHEMA extensions;

-- ========================================
-- 2. Create table to log weekly job runs (if not exists)
-- ========================================
CREATE TABLE IF NOT EXISTS public.weekly_jobs_log (
...
ALTER TABLE public.weekly_jobs_log ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'weekly_jobs_log' AND policyname = 'Only admins can view job logs'
  ) THEN
    CREATE POLICY "Only admins can view job logs" ON public.weekly_jobs_log
    FOR SELECT USING (public.has_role(auth.uid(), 'admin'));
  END IF;
END $$;

-- ========================================
-- 3. Drop and recreate get_admin_kpis with test stats
-- ========================================
DROP FUNCTION IF EXISTS public.get_admin_kpis(DATE, DATE, UUID, TEXT);

CREATE FUNCTION public.get_admin_kpis(
  _start_date DATE,
  _end_date DATE,
...
BEGIN
  ...
  WITH teacher_stats AS (
    ...
    FROM public.lesson_records lr
    JOIN public.profiles p ON p.id = lr.teacher_id
    WHERE lr.lesson_date BETWEEN _start_date AND _end_date
      AND lr.submitted = true
      AND (_teacher_id IS NULL OR lr.teacher_id = _teacher_id)
      AND (_subject IS NULL OR lr.subject::text = _subject)
