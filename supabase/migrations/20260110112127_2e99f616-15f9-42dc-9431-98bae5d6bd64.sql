-- Create a wrapper function for scheduled execution that bypasses admin check
CREATE OR REPLACE FUNCTION public.generate_weekly_reports_scheduled(_week_start DATE, _week_end DATE)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- No authorization check - this is for scheduled execution only
  -- The edge function should only call this with service role
  ...
  FOR s IN SELECT id, name FROM public.students LOOP
    ...
    FOR hyugang_rec IN
      SELECT lr.lesson_date, lr.subject
      FROM public.lesson_records lr
      WHERE lr.student_id = s.id
        AND lr.lesson_date BETWEEN _week_start AND _week_end
        AND lr.submitted = true
    ...
    FOR exception_rec IN
      SELECT lr.lesson_date, lr.subject, lr.lesson_types, lr.attendance_status
      FROM public.lesson_records lr
      WHERE lr.student_id = s.id
        AND lr.lesson_date BETWEEN _week_start AND _week_end
        AND lr.submitted = true
    ...
    ELSE
      ...
      FOR subj_name IN SELECT unnest(ARRAY['수학', '과학', '영어', '국어']) LOOP
        ...
        IF subj_lessons > 0 THEN
          ...
          test_sentence := '';
          FOR test_rec IN
            SELECT test_title, test_result_text, test_result, english_pass_fail, test_date
            FROM public.lesson_records
            WHERE student_id = s.id
              AND lesson_date BETWEEN _week_start AND _week_end
              AND submitted = true
          ...
          LOOP
            ...
            END IF;
            test_sentence := test_sentence || COALESCE(test_rec.test_title, '테스트') || ' – ' || test_rec.test_result_text;
            IF subj_name = '영어' AND test_rec.english_pass_fail IS NOT NULL THEN
              IF test_rec.english_pass_fail = 'pass' THEN
                test_sentence := test_sentence || ' (통과)';
              ELSIF test_rec.english_pass_fail = 'fail' THEN
