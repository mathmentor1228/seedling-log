-- Add lesson_types and attendance_status columns to lesson_records
ALTER TABLE public.lesson_records 
ADD COLUMN IF NOT EXISTS lesson_types TEXT[] DEFAULT '{"정규수업"}',
ADD COLUMN IF NOT EXISTS attendance_status TEXT[] DEFAULT '{"정상등원"}';

-- Update the generate_weekly_reports function to include "이번 주 특이사항" section
...
BEGIN
  ...
  FOR s IN SELECT id, name FROM public.students LOOP
    ...
    -- Build exception lines for 이번 주 특이사항
    -- Find records where lesson_types != ['정규수업'] OR attendance_status != ['정상등원']
    FOR exception_rec IN
      ...
      WHERE lr.student_id = s.id
        ...
        AND lr.submitted = true
        AND (
          -- attendance_status contains any exception
          (lr.attendance_status && ARRAY['지각', '조퇴', '인정결석', '무단결석', '보충불가'])
          OR
          -- lesson_types contains any exception
    ...
    LOOP
      ...
      BEGIN
        ...
        -- Add attendance_status exceptions
        ...
        -- Build line if we have exceptions
    ...
    IF total_lessons_i = 0 THEN
      ...
      risk_txt := 'low';
    ELSE
      -- Generate week-over-week comparison sentence
      ...
      FOR subj_name IN SELECT unnest(ARRAY['수학', '과학', '영어', '국어']) LOOP
        ...
        IF subj_lessons > 0 THEN
          ...
          -- G) Build test sentence if test_result_text exists
          test_sentence := '';
          FOR test_rec IN
          ...
          LOOP
            ...
            -- Append pass/fail ONLY for English
          ...
          END IF;
          -- Include test results for student
          IF test_sentence != '' THEN
      ...
      IF array_length(exception_lines, 1) > 0 THEN
        ...
        -- Also add a lighter version to student message
