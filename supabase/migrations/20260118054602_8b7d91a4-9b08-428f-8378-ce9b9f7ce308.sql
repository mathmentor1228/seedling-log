-- Fix the weekly report generation functions:
-- 1. Add _student_ids parameter for per-student generation
-- 2. Fix ENUM comparison by properly casting subject column to TEXT

-- Drop and recreate generate_weekly_reports with _student_ids parameter
...
BEGIN
  ...
  LOOP
    ...
    -- Build subject breakdown and messages (cast subject to text for comparison)
    FOR subj_name IN 
    ...
    -- Upsert into weekly_reports (idempotent)
...
-- Recreate generate_weekly_reports_scheduled with _student_ids parameter and fixed casting
...
BEGIN
  ...
  LOOP
    ...
    -- Build 휴강 lines (cast subject to text)
    FOR hyugang_rec IN
      SELECT lr.lesson_date, lr.subject::text AS subject_txt
      FROM public.lesson_records lr
      WHERE lr.student_id = s.id
    ...
    -- Build exception lines (cast subject to text)
    FOR exception_rec IN
      SELECT lr.lesson_date, lr.subject::text AS subject_txt, lr.lesson_types, lr.attendance_status
      FROM public.lesson_records lr
      WHERE lr.student_id = s.id
    ...
    -- Build subject breakdown (cast subject to text for comparison)
    FOR subj_name IN 
      SELECT DISTINCT subject::text FROM public.lesson_records 
      WHERE student_id = s.id AND lesson_date BETWEEN _week_start AND _week_end AND submitted = true
    LOOP
