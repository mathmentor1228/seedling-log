...
BEGIN
  ...
  -- Remove only pre-existing, history-free rows omitted by the submitted list.
  -- This must happen before inserts so newly inserted rows are never mistaken for removals.
  WITH del AS (
